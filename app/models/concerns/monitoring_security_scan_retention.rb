...
module MonitoringSecurityScanRetention
  ...
  class_methods do
    def retention_days
      fetch_setting('security_retention_days',
                    DEFAULT_SETTINGS[:security_retention_days]) { |value| value.to_i if value.to_i.positive? }
    end
